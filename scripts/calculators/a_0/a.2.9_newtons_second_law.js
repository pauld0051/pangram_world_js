...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function disablePredefinedOptions(disable) {
    ...
    predefinedOptions.forEach((optionValue) => {
      const option = unitGravitySelect.querySelector(
  ...
  function clearLockedInput() {
    if (calculatedField) {
      const inputField = document.getElementById(
        `input${calculatedField.charAt(0)}`
      );
      inputField.value = "";
  ...
  [forceInput, massInput, gravityInput, momentumInput, timeInput].forEach(
    (input) => {
  ...
  unitForceSelect.addEventListener("change", function () {
    ...
    forceInput.value = formatResult(force, [
      force,
      massInput.value,
      gravityInput.value,
      momentumInput.value,
  ...
  unitMassSelect.addEventListener("change", function () {
    ...
    // Recalculate and format the result with significant figures if applicable
    massInput.value = formatResult(mass, [
      forceInput.value,
      massInput.value,
      gravityInput.value,
      momentumInput.value,
  ...
  unitMomentumSelect.addEventListener("change", function () {
    ...
    // Recalculate and format the result with significant figures if applicable
    momentumInput.value = formatResult(momentum, [
      forceInput.value,
      massInput.value,
      gravityInput.value,
      momentumInput.value,
  ...
  unitTimeSelect.addEventListener("change", function () {
    ...
    // Recalculate and format the result with significant figures if applicable
    timeInput.value = formatResult(time, [
      forceInput.value,
      massInput.value,
      gravityInput.value,
      momentumInput.value,
  ...
  [forceInput, massInput, gravityInput, momentumInput, timeInput].forEach(
    (input) => {
      input.addEventListener("click", () => {
        if (input.readOnly) {
          // Change to check if input is readonly
  ...
  function showTooltipModal(message) {
    const tooltipModal = new bootstrap.Modal(
      document.getElementById("errorTooltipModal")
    );
...
katex.render(
  "\\large \\color{#333}{F = ma = \\frac{\\Delta p}{\\Delta t}}",
  document.getElementById("equation1"),
  options
);
katex.render(
  "\\large \\color{#333}{m = \\frac{F}{a} = \\frac{\\Delta p}{a \\cdot \\Delta t}}",
  document.getElementById("equation2"),
  options
);
katex.render(
  "\\large \\color{#333}{a = \\frac{F}{m} = \\frac{\\Delta p}{m \\cdot \\Delta t}}",
  document.getElementById("equation3"),
  options
);
katex.render(
  "\\large \\color{#333}{\\Delta p = F \\cdot \\Delta t = ma \\cdot \\Delta t}",
  document.getElementById("equation4"),
  options
);
katex.render(
  "\\large \\color{#333}{\\Delta t = \\frac{\\Delta p}{F} = \\frac{\\Delta p}{ma}}",
