...
document.addEventListener("DOMContentLoaded", function () {
  const generateNumberContainer = document.getElementById(
  ...
  generateNumberContainer.addEventListener("click", function (event) {
    ...
    if (hasDecimal) {
      ...
      const integerPart = Math.floor(
        Math.random() * Math.pow(10, numDigits - numDecimalPlaces)
      );
      const fractionalPart = Math.floor(
        Math.random() * Math.pow(10, numDecimalPlaces)
      )
        .toString()
        .padStart(numDecimalPlaces, "0");
      generatedNumber =
        integerPart === 0
          ? `0.${fractionalPart}`
  ...
  revealResultBtn.addEventListener("click", function (event) {
    ...
    if (isNaN(userSigFigs)) {
      testResultOutput.textContent =
        "Please enter a valid number of significant figures.";
      return;
    }
